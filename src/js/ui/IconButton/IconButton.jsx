  ...
  return (
    <ButtonBase
      className={clsx(
        'icon-btn',
        {
          [`icon-btn-color${capitalize(color)}`]: color !== 'default',
          'icon-btn-disabled': disabled,
          [`icon-btn-size${capitalize(size)}`]: size !== 'medium',
          'icon-btn-edgeStart': edge === 'start',
          'icon-btn-edgeEnd': edge === 'end',
        },
        className,
      )}
      centerRipple
      ...
      {...other}
    >
      <span className="icon-btn-label">{children}</span>
    </ButtonBase>
  );
});
